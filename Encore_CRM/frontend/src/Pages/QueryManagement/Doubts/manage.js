...
import { MdKeyboardArrowLeft, MdRemoveRedEye, MdKeyboardArrowDown, MdKeyboardArrowUp } from "react-icons/md";
...
function DoubtManage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setLoading(true);
    const url = `${API_BASE_URL}fetch-doubt-order-details?&page=${currentPage - 1}&query=${filters.search_text}&myob=${filters.myobStatus}&sortField=${sorting.sortField}&sortDirection=${
                      ...
                      orders.fetchedItems.map(item => (
                        <React.Fragment key={item._id}>
                          ...
                          >
                            ...
                            <TableCell align="left">
                              <MyDiv>{getStatusBadge(item.order_doubt_status)}</MyDiv>
                            </TableCell>

...
function OrderDesignersDashboard() {
  ...
  const loadOrders = useCallback(
    async allOrderCurrentPage => {
      setLoading(true);
      const url = `${API_BASE_URL}designer-dashboard-order-list?page=${allOrderCurrentPage - 1}&query=${searchData.search_text}`;
      try {
  ...
  const loadSpecificDesignerOrders = useCallback(async specificDesignerOrderCurrentPage => {
    // setLoading(true);
    const url = `${API_BASE_URL}specific-designer-order-list?page=${specificDesignerOrderCurrentPage - 1}`;
    try {
      const response = await axios.get(url, { headers: { "x-access-token": localStorage.getItem("token"), Accept: "application/json", "Content-Type": "application/json" } });
  ...
  return (
    <React.Fragment>
      ...
      <Row className="GeneralHeading designerDashboardBg mt-2 SearchBar">
        <Col md={8}>
          <HeadingFive>Your Pending Orders</HeadingFive>
        </Col>
      </Row>
      <Row>
        <Col md={12}>
      ...
      <Row className="GeneralHeading designerDashboardBg mt-3 SearchBar">
        <Col md={9}>
          <HeadingFive>All Available Orders</HeadingFive>
        </Col>
        <Col md={3}>
...
export default OrderDesignersDashboard;

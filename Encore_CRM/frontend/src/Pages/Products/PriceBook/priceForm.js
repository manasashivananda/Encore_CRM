...
export default function FormMaterialPrice({ handleClose, materialInfo }) {
  let { id } = useParams();
  ...
  useEffect(() => {
    const loadSpecificMaterialPrice = async () => {
      try {
        setLoading(true);
        ...
        setCusSpeMatPrice(response.data[0].pricedetails);
        setLoading(false);
      } catch (error) {
        swal.fire({
    ...
    loadSpecificMaterialPrice();
  }, [id, materialInfo]);

  const onSubmit = async values => {
    setBtnLoading(true);
    const url = API_BASE_URL + "add-specific-material-pricebook/" + id;
    const method = "post";
    var data = values;
    try {
      await axios({ method, url, data, headers: { "x-access-token": localStorage.getItem("token"), Accept: "application/json", "Content-Type": "application/json" } });
      swal.fire({
  ...
  return (
    <div className="DrawerRight priceBookDrawer">
      <MyDiv className="grid-margin general-form stretch-card">
        <Card md={12}>
          <Card.Header>
            <HeadingFour className="card-title">{materialInfo ? materialInfo.name : ""} - Price Book</HeadingFour>
          </Card.Header>
          <Card.Body>
            {loading ? (
              ...
              <>
                <MyDiv className="DrawerForm">
                  <Row className="PriceBookHeader">{speMetPrice.headers ? speMetPrice.headers.map((HeaderItem, index) => <Col key={index}>{HeaderItem.value}</Col>) : null}</Row>

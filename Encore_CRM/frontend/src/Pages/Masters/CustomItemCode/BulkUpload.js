...
const CustomerItemBulkUpload = ({ cusId, handleResponse }) => {
  ...
  const handleSubmit = async e => {
    ...
    const url = customerId ? `${API_BASE_URL}bulk-import-custom-itemcode-customer-priced/${customerId}` : `${API_BASE_URL}bulk-import-custom-itemcode-default-priced`;
    const method = "post";
    ...
    try {
      ...
      if (response.status === 200) {
        setLoading(false);
        swal.fire({
          text: "Successfully Added Default Price",
          icon: "success",
          type: "success",
        });
        setFileData(null);
    ...
    <MyDiv className="float-end">
      ...
      <MyDiv className="d-flex justify-content-end align-items-center">
        <MyDiv md={5} className="pe-0">
          <StrongTag className="me-2 float-start">Custom Item Bulk Upload :</StrongTag>
...
export default CustomerItemBulkUpload;

...
import { setFilters, resetFilters } from "../../store/designReportSlice";
import Row from "react-bootstrap/Row";
import Col from "react-bootstrap/Col";
import { Link, useNavigate } from "react-router-dom";
import { HeadingTwo, HeadingFour, MyDiv, SpanTag, HeadingThree, PTag, LabelTag, getFormattedDeliveryTime } from "../Common/Components";
...
function DesignerReport() {
  ...
  const { filters } = useSelector(state => state.designReport);
  ...
  const loadReport = useCallback(async () => {
    ...
    setLoading(true);
    const url = `${API_BASE_URL}design-reports-generate?page=${currentPage - 1}&orderUID=${filters.search_text}&fromTime=${fromTimeUTC}&toTime=${toTimeUTC}&orderDesigner=${filters.designer}`;
    try {
  ...
  const loadEmployee = async () => {
    if (RolePermission?.DesignReport?.add === "1") {
      ...
      try {
        ...
        swal.fire({
          text: error.response || "Data load Failed",
          icon: "error",
        });
      }
    }
  ...
  const loadFilterReportExport = async () => {
    if (cancelToken.current) {
      cancelToken.current.cancel("Operation canceled due to new request.");
    }
    const source = axios.CancelToken.source();
    ...
    const url = `${API_BASE_URL}design-reports-generate-export?orderUID=${filters.search_text}&fromTime=${fromTimeUTC}&toTime=${toTimeUTC}&orderDesigner=${filters.designer}`;
    try {
      const response = await axios.get(url, {
        headers: {
          "x-access-token": localStorage.getItem("token"),
  ...
  const prepareDataForExport = data => {
    return data.map(item => {
      return {
        ...
        "Designed Person": item.designerFullName,
        "Design Start Time": item.order_design_assigned_time ? moment(item.order_design_assigned_time).format("DD-MM-YYYY hh:mm a") : "-",
        "Design End Time": item.order_design_completed_time ? moment(item.order_design_completed_time).format("DD-MM-YYYY hh:mm a") : "-",
        "Delivery date": item.order_delivery_date_str ? item.order_delivery_date_str : "",
        Pieces: item.Order_Flashing_Count,
        Drawings: item.shapeIDCount,
  ...
  const exportToExcel = dataForExport => {
    ...
    XLSX.writeFile(wb, "Designer-report.xlsx");
  };
          ...
          <MyDiv className="searchBar GeneralHeading mt-2 d-block">
            <Row>
              ...
              {RolePermission?.DesignReport?.add === "1" ? (
                <Col md={2}>
                  <TextField select variant="outlined" size="small" label="Designer" name="designer" value={filters.designer} SelectProps={{ onChange: handleFieldChange }} className="form-control">
                  ...
                  <Table className="bgGrey" aria-label="Order table">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell align="left">Designed Person</TableCell>
                        <TableCell align="left">Design Start Time</TableCell>
                        <TableCell align="left">Design End Time</TableCell>
                        <TableCell align="left">Delivery date</TableCell>
                        <TableCell align="left">Pieces</TableCell>
                        <TableCell align="left">Drawings</TableCell>
                        ...
                        filteredOrders.orderReportPagination.map(item => (
                          <React.Fragment key={item._id}>
                            <TableRow>
                              ...
                              <TableCell align="left" component="th" scope="row">
                                {item.order_design_assigned_time ? moment(item.order_design_assigned_time).format("DD-MM-YYYY hh:mm a") : "-"}
                              </TableCell>
                              <TableCell align="left" component="th" scope="row">
                                {item.order_design_completed_time ? moment(item.order_design_completed_time).format("DD-MM-YYYY hh:mm a") : "-"}
                              </TableCell>
                              <TableCell align="left" component="th" scope="row">

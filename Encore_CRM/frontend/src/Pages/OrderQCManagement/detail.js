...
import { HeadingFour, MyDiv, LabelTag, SpanTag, GetDayFromDate, HeadingThree, getFormattedDeliveryTime } from "../Common/Components";
...
function OrderQualityCheckingDetail() {
  let { id } = useParams();
  ...
  const handleOrderSelfAssign = async () => {
    ...
    try {
      const url = `${API_BASE_URL}assign-order-to-qced/${id}/${userId}`;
      await axios.patch(
        url,
        {},
        {
      ...
      swal.fire({
        text: "This Order Successfully Assigned to You for QC",
        icon: "success",
        type: "success",
      });
      loadSpecificOrder();
              ...
              <React.Fragment key={item._id}>
                <Row>
                  ...
                  <Col md={12} className="mb-2">
                    <MyDiv className="ProfileBasicDetail order-profile qcDashboardBg">
                      <Card className="ProfileBasicDetailLeftHeader pt-0">
                        ...
                        >
                          <Link className="arrow-btn" to="/quality-checking">
                            <MdKeyboardArrowLeft />
                          </Link>
                          <Col md="6">
                            <HeadingThree>
                        ...
                        <Row>
                          ...
                          <Col md={2} className="d-flex flex-column align-items-end">
                            {item.order_design_stage === "2" && item.order_status !== "Order Cancelled" && !item.order_hold ? (
                              <Button onClick={e => handleOrderSelfAssign()} className="btn primary-btn mb-2" startIcon={<MdAssignmentInd />}>
                                Assign to Me for QC
                              </Button>
                            ) : (
                              ""
...
export default OrderQualityCheckingDetail;
